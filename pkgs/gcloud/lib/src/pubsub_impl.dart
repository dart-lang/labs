...
class _PullEventImpl implements PullEvent {
  ...
  _PullEventImpl(
    ...
    pubsub.PullResponse response,
  ) : _response = response,
      message = _PullMessage(
        response.receivedMessages![0].message!,
...
class _PushEventImpl implements PushEvent {
  ...
  factory _PushEventImpl.fromJson(String json) {
    ...
    return _PushEventImpl(
      _PushMessage(data, labels, orderingKey),
      subscription,
    );
  }
}
...
class _SubscriptionPageImpl implements Page<Subscription> {
  ...
  Future<Page<Subscription>> next({int? pageSize}) {
    ...
    return _api._listSubscriptions(_topic, pageSize_, _nextPageToken).then((
      response,
    ) {
