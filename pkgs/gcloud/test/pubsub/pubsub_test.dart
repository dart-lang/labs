  ...
  group('api', () {
    ...
    group('subscription', () {
      ...
      test('create-error', () {
        ...
        for (var name in badSubscriptionNames) {
          expect(
            () => api.createSubscription(name, 'test-topic'),
            throwsArgumentError,
          );
        }
        for (var name in badTopicNames) {
          expect(
            () => api.createSubscription('test-subscription', name),
            throwsArgumentError,
          );
        }
      });
        ...
        void addSubscriptions(
          ...
          for (var i = 0; i < count; i++) {
            response.subscriptions!.add(
              pubsub.Subscription()..name = 'subscription-${first + i}',
            );
          }
        }
          ...
          .then(
            expectAsync1((topic) {
              ...
              registerPublish(mock, 4, (request) {
                ...
                return mock.respond(
                  pubsub.PublishResponse()..messageIds = ['0'],
                );
              });
                              ...
                              return topic
                                  ...
                                  .then(
                                    expectAsync1((result) {
                                      ...
                                      return topic
                                          .publish(
                                            Message.withBytes(messageBytes),
                                          )
                                          .then(
                                            expectAsync1((result) {
                                              expect(result, isNull);
          ...
          .then(
            expectAsync1((topic) {
              ...
              registerPublish(mock, 4, (request) {
                ...
                expect(
                  request.messages![0].attributes!.length,
                  attributes.length,
                );
