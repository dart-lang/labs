  ...
  if [[ ${EXIT_CODE} -ne 0 ]]; then
    ...
    for TASK in "$@"; do
      ...
      test_1)
        ...
        ;;
      test_2)
        echo 'dart test --test-randomize-ordering-seed=random --platform chrome -c dart2wasm'
        dart test --test-randomize-ordering-seed=random --platform chrome -c dart2wasm || EXIT_CODE=$?
        ;;
      *)
